 ...
 * Threshold 1 = off: the entropy ceiling is 1, so nothing qualifies and there
 * is no point scanning. Mutates the grid in place; the caller redraws.
 */
const maybeReseed = () => {
    const st = db.deref();
    if (st.entropyThreshold >= 1) return;
